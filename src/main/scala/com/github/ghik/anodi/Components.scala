...
object Component {
  ...
  def destroyAll(components: Seq[Component[_]])(implicit ec: ExecutionContext): Future[Unit] = {
    val reverseGraph = new mutable.HashMap[Component[_], ListBuffer[Component[_]]]
    val terminals = new mutable.HashSet[Component[_]]
    GraphUtils.dfs(components)(
      _.dependencies.toList,
      onEnter = { (c, _) =>
    ...
    val destroyFutures = new mutable.HashMap[Component[_], Future[Unit]]
